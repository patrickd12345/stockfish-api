...
async function migrate() {
  ...
  console.log('Creating Blunder DNA tables...')
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_blunder_dna_drills_user_tag ON blunder_dna_drills (lichess_user_id, pattern_tag)`
  await sql`CREATE INDEX IF NOT EXISTS idx_blunder_dna_attempts_user_time ON blunder_dna_attempts (lichess_user_id, attempted_at DESC)`
  await sql`CREATE INDEX IF NOT EXISTS idx_blunder_dna_patterns_user_score ON blunder_dna_patterns (lichess_user_id, weakness_score DESC)`

  console.log('Blunder DNA migration complete.')
  process.exit(0)
}
