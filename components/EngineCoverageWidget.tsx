...
export default function EngineCoverageWidget({
  ...
  const access = useFeatureAccess('engine_coverage')
  ...
  }, [coverageUrl, pollMs, queueUrl, access.allowed])
  ...
  if (!access.allowed) {
    return (
      <div
        ...
        title={getFeatureErrorMessage('engine_coverage', access.reason ?? 'capability')}
      >
        <div className="text-sage-400 text-xs font-bold">Engine coverage unavailable</div>
        <div className="mt-1 text-sage-500 text-[11px]">
          {getFeatureErrorMessage('engine_coverage', access.reason ?? 'capability')}
        </div>
      </div>
    )
  }
    ...
    <div className={`border border-white/5 bg-sage-800/50 backdrop-blur-sm rounded-xl ${compact ? 'p-2 min-w-[220px]' : 'p-3 min-w-[260px]'} shadow-sm`}>
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        <button
          ...
          disabled={isResuming || !access.allowed}
          ...
          title={!access.allowed ? 'Feature Engine Coverage is not supported in this environment.' : undefined}
        >
